...
namespace CapaPresentacion.Formularios
{
    public partial class frmProfesionales : Form
    {
        ...
        private void frmProfesionales_Load(object sender, System.EventArgs e)
        {
            ...
            {
                dgvProfesionales.Rows.Add(new object[] { "", item.id_Prof, item.Matricula, item.ApelNombres, item.TipoDoc, item.NumeroDoc, item.Sexo, item.Telefono, item.Email, item.Estado,
                                                item.FecEstado, item.Obs, item.UserRegistro, item.FechaRegistro });
            }
        ...
        private void btnGuardar_Click(object sender, EventArgs e)
        {
            ...
            if (respuesta == "OK")
            {
                ...
                if (cE_Profesionales.id_Prof == 0)
                {
                    ...
                    if (idProfesional != 0)
                    {
                        dgvProfesionales.Rows.Add(new object[] {"",idProfesional,txtMatricula.Text,txtApelNombres.Text,cboTipoDoc.Text,txtNumeroDoc.Text,cboSexo.Text,
        ...
        private void dgvProfesionales_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
            if (dgvProfesionales.Columns[e.ColumnIndex].Name == "Seleccionar")
            {
                ...
                if (indice >= 0)
                {
                    ...
                    cboTipoDoc.Text = dgvProfesionales.Rows[indice].Cells["TipoDoc"].Value.ToString();
                    txtNumeroDoc.Text = dgvProfesionales.Rows[indice].Cells["NumeroDoc"].Value.ToString();
